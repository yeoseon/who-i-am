...
### 금융 솔루션 CBP :: 기반지식과 요건이 명확하지 않은 상태에서의 표준 속성 API 개발 진행    
...
* 개발 과정에서 기존 기능이 너무 많은 역할을 수행하고 있기에 분리하여 재사용 하는 방식으로 Refactoring을 진행하고, 분리한 단위 기능을 이용해 요건을 개발  
...
* 중복되는 금융 거래 발생시, 커스터미이징된 Exception을 발생시켜 고객에게 중복거래임을 알리는 기능 개발  
* CBP의 시스템 구조과 관련되어 아키텍쳐팀과 소통하며 개발 진행  
* 간단한 요건만을 가지고 잘 파악되지 않던 제품의 시스템 구조를 문서를 기반으로 이해, 해당 기능 사용 개발자들과의 회의 주도, 질문을 바탕으로한 소통을 진행하며 개발함  
* 클린 코드 구현을 위해 테스트 코드를 기반으로 개발하려고 노력함  
* 그 결과 에러 발생 사항 없었음  
* 중복 거래 메시지 처리를 위해 AngularJS로 구현된 기존 화면 소스코드에 구현하는 역할까지 수행  
